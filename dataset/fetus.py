...
class FETUSSemiDataset(Dataset):
    def __init__(self, json_file_path, mode, size=None, n_sample=None):
        ...
        self.root = "/root/autodl-tmp/FETUS2026/data/"
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'valid':
            ...
            image_h5_file = self.root + "images/" + case
            label_h5_file = image_h5_file.replace(".h5", "_label.h5").replace("images/", "labels/")
            ...
            return (image, torch.from_numpy(image_view_id).long() - 1, torch.from_numpy(mask).long(), torch.from_numpy(label).long())
