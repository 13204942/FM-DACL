...
parser.add_argument('--ict_alpha', type=float, default=0.5,
                    help='ict_alpha')
# consistency_weight_cps = get_current_consistency_weight(iter_num // rampup_rate, args.cps)  # lambda
# consistency_weight_mt = get_current_consistency_weight(iter_num // rampup_rate, args.consistency)  # tau
# contra_weight_contra = get_current_consistency_weight(iter_num // rampup_rate, args.contrastive)  # beta
...
def build_optimizer(model_name, model):
    ...
    if model_name == "cnn":
        # optimizer = optim.SGD(model.parameters(), lr=args.base_lr, momentum=0.9, weight_decay=1e-4)
        optimizer = optim.Adam(model.parameters(), lr=args.base_lr, weight_decay=0.01)
        base_lrs = [args.base_lr]   # Only 1 group
        return optimizer, base_lrs  
...
def train(args, snapshot_path):
    ...
    train_loader_l = DataLoader(db_train_l, batch_size=args.labeled_bs, shuffle=True, num_workers=1)
    ...
    #                        momentum=0.9, weight_decay=0.0001)
    optimizer1, _ = build_optimizer('mit', model1)
    optimizer2, _ = build_optimizer('cnn', model2)
    ...
    for epoch_num in iterator:
        for i_batch, ((image_x, view_x, mask_x, class_label_x)) in enumerate(trainloader):
            ...
            unlabeled_image_x_0 = unlabeled_image_x[0:unlabeled_bs//2, ...]
            unlabeled_image_x_1 = unlabeled_image_x[unlabeled_bs//2:, ...]
            # print(unlabeled_image_x_0.shape) # [B, 1, 256, 256]
            # print(unlabeled_image_x_1.shape) # [B, 1, 256, 256]

            # Mix images
            batch_u_x_mixed = unlabeled_image_x_0 * (1.0 - ict_mix_factors) + unlabeled_image_x_1 * ict_mix_factors
            # input_volume_batch = torch.cat([labeled_volume_batch, batch_u_x_mixed], dim=0)
            # mix_outputs1, _ = model1(batch_u_x_mixed)
            mix_outputs2, mix_outputs2_cls = model2(batch_u_x_mixed)
            # mix_outputs_soft1 = torch.softmax(mix_outputs1, dim=1)
            mix_outputs_soft2 = torch.softmax(mix_outputs2, dim=1)
            with torch.no_grad():
                ema_unlabeled_image_x_0, ema_unlabeled_cls_0 = ema_model(unlabeled_image_x_0)
                ema_unlabeled_image_x_1, ema_unlabeled_cls_1 = ema_model(unlabeled_image_x_1)
                ema_output_u_x_0 = torch.softmax(ema_unlabeled_image_x_0, dim=1)
                ema_output_u_x_1 = torch.softmax(ema_unlabeled_image_x_1, dim=1)                    
                batch_ema_pred_mixed = ema_output_u_x_0 * (1.0 - ict_mix_factors) + ema_output_u_x_1 * ict_mix_factors
                ict_mix = ict_mix_factors.squeeze()[:, None]   # shape → [3,1]
                # print(ict_mix.shape)
                batch_ema_pred_mixed_cls = ema_unlabeled_cls_0 * (1.0 - ict_mix) + ema_unlabeled_cls_1 * ict_mix
                mix_ema_cls = torch.sigmoid(batch_ema_pred_mixed_cls)
            ...
            # if iter_num < 10:                
                ...
                mix_ema_cls = mix_ema_cls.view(mix_ema_cls.size(0), -1).float()
                u_mix_cls_loss2_map = F.binary_cross_entropy_with_logits(mix_outputs2_cls, mix_ema_cls.float(), reduction="none", pos_weight=pos_weight)
                u_mix_cls_loss2 = u_mix_cls_loss2_map.mean(dim=1).mean()
                # u_mix_cls_loss2 = masked_bce_with_logits(mix_outputs2_cls, mix_ema_cls.float(), mask_x_allowed[args.labeled_bs:], pos_weight=pos_weight)                
                # consistency_loss1 = torch.mean((mix_outputs_soft1 - batch_ema_pred_mixed) ** 2)
                consistency_loss2 = torch.mean((mix_outputs_soft2 - batch_ema_pred_mixed) ** 2)
            ...
            model1_loss = loss1 + loss1_cls + consistency_weight_cps * 0.5 * (pseudo_supervision1 + pseudo_supervision1_cls) #+ consistency_weight_mt * consistency_loss1
            model2_loss = loss2 + loss2_cls + consistency_weight_cps * 0.5 * (pseudo_supervision2 + pseudo_supervision2_cls) + consistency_weight_mt * 0.5 * (consistency_loss2 + u_mix_cls_loss2)
