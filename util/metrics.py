...
def dice_score(pred, target, num_classes=2):
    # print('-'*20)
    # print(pred.shape)
    ...
    # print('-'*20)
    pred = F.one_hot(pred.argmax(dim=1).long(), num_classes).permute(0,3,1,2)  # (BS,2,224,224)
    target = F.one_hot(target.long(), num_classes).permute(0,3,1,2)  # (BS,2,224,224)
    target = target.float()
